...
import { PaperAirplaneIcon, XMarkIcon, ArrowUturnRightIcon } from '@heroicons/react/24/outline'
            ...
            <>
              <div className="p-8 container">
                ...
                {comments && comments.length > 0 && (
                  <Comments comments={comments} replyingTo={replyingTo} setReplyingTo={setReplyingTo}/>
                )}
                <div>
                  {replyingTo &&
                  <div className="flex items-start space-x-2 my-2">
                    ...
                    <span className="font-mono text-sm bg-black">{[...comments].find(x => x.id == replyingTo).text}</span>
                    </p>
        ...
        {comments.map((comment, i) => (
          <>
            {comment.parent_comment == null && (
              ...
              <Comment key={i} comment={comment} comments={comments} replyingTo={replyingTo} setReplyingTo={setReplyingTo}/>
              </div>
            )}
          </>
        ))}
...
function Comment({ comment, comments, replyingTo, setReplyingTo }) {
  const colors = [
    'bg-blue-500', 'bg-red-500', 'bg-indigo-500'
  ]
    ...
    <li className="">
      <div className="flex space-x-2 items-center justify-between">
        <div className="inline-flex items-center space-x-2">
          <div className={`rounded-full text-sm font-mono w-10 h-10 flex justify-center items-center ${colors[Math.floor(Math.random() * colors.length)]}`}>
            {user ? (
              <>
        ...
        <div className="text-sm text-blue-500">{formatRelative(new Date(comment.created_at), new Date())}</div>
      </div>

      <div className="py-1 pl-8  relative">
        <div onClick={() => setReplyingTo(comment.id)} className="px-4 mb-4 flex group cursor-pointer justify-between">
          <p>{comment.text}</p>
          <div className={`group-hover:block ${comment.id != replyingTo && `hidden`} w-4 h-4`}>
          <ArrowUturnRightIcon />
          </div>
        </div>
        ...
        <ul className="my-2">
          {childComments.map((child, i) => (
            <>
              <Comment comment={child} key={i} comments={comments} replyingTo={replyingTo} setReplyingTo={setReplyingTo} />
            </>
          ))}
        </ul>
      </div>
